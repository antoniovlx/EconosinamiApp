...
export class UtilService {
  ...
  configuracionLoaded$ = new Subject<Configuracion>();

  configuracion: Configuracion;


  constructor(private configuracionRepository: ConfiguracionRepository, private appService: AppService,
  ...
  getConfiguracion() {
    this.configuracionRepository.getConfiguracion().subscribe(configuracion => {
      this.configuracion = configuracion;
      this.configuracionLoaded$.next(this.configuracion);
    })
  }
  ...
  isConfiguracionLoaded() {
    return this.configuracionLoaded$.asObservable();
  }

  getMoneda() {
    const moneda = this.configuracion.moneda;
    return { moneda: moneda.slice(moneda.length - 2, moneda.length - 1) }
  }
  ...
  getHeaderHtml() {
    ...
    htmlContent += "<html>\n";
    htmlContent += "<head>\n";
    htmlContent += "<title>Simulaci&oacute;n EconoSINAMI 2.0</title>\n";
    htmlContent += "<style>\n";
  ...
  async writeAndOpenFile(data: Blob, fileName: string) {
    ...
    reader.onloadend = async function () {
      ...
      try {
        ...
        that.fileOpener.open(result.uri, data.type)
          .then(() => {
